...
export const updateTask = async (req: Request, res: Response) => {
  try {
    ...
    const project = task.project as any; // Cast para acceder a las propiedades
    ...
    if (updateData.estimatedTime) {
      if (user.role === 'worker' && !project.allowWorkerEstimation) {
        return res.status(403).json({ message: 'No tienes permiso para estimar tareas.' });
      }
    }

    if (updateData.assignee && !project.members.includes(updateData.assignee)) {
...
export const deleteTask = async (req: Request, res: Response) => {
  try {
    ...
    if (!(task.project as any).members.includes(userId)) {
      return res.status(403).json({ message: 'Acción no autorizada.' });
    }
